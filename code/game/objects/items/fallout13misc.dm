...
/obj/item/flag/redguard
	name = "Red Guard flag"
	desc = "A red banner with star on top of hammer and sickle. There's some text on the sides."
	icon_state = "redguardflag"
	item_state = "redguardflag"
	faction = "Red Guard"
...
/obj/item/flag/attackby(obj/item/I, mob/user, params)
	if(istype(I, /obj/item/stack/sheet/leather) && item_state == "emptyflag")
		...
		if(do_after(user, 60, target = src))
			...
			if(H.use(1))
				var/flag = alert(user, "Please choose which faction flag you wish to create.", "Flag type", "NCR", "Legion", "Red Guard")
				switch(flag)
					if("NCR")
						name = "NCR flag"
						desc = "A flag with a two headed bear, the symbol of the New California Republic."
